    ...
    try {
      ...
      for (const file of files) {
        ...
        for (const page of pages) {
          ...
          // ⚡ Crop top half (Meesho labels like Flipkart)
          const cropBox = {
            left: 0,
            bottom: halfHeight,
            right: width,
      ...
      const fileName = `Meesho-Label-${new Date().toISOString().slice(0, 19)}.pdf`;
  ...
  return (
    <div className="p-8 border rounded-lg bg-white shadow-md max-w-2xl mx-auto">
      ...
      <label className="cursor-pointer bg-pink-600 text-white px-6 py-3 rounded-lg shadow-md hover:bg-pink-700 transition">
        {loading ? "⏳ Processing..." : "Upload Meesho PDF"}
        <input type="file" accept="application/pdf" multiple onChange={handleFiles} className="hidden" disabled={loading}/>
      </label>

      {success && <p className="mt-4 text-green-600 font-semibold animate-bounce">✅ Meesho Label cropped successfully!</p>}
